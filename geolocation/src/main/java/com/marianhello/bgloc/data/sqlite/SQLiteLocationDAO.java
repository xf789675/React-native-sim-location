...
public class SQLiteLocationDAO implements LocationDAO {
  ...
  private Collection<BackgroundLocation> getLocations(String whereClause, String[] whereArgs) {
    ...
    String[] columns = {
      ...
      LocationEntry.COLUMN_NAME_MESSAGE,
    };
  ...
  private BackgroundLocation hydrate(Cursor c) {
    ...
    l.setMessage(c.getString(c.getColumnIndex(LocationEntry.COLUMN_NAME_MESSAGE)));
  ...
  private ContentValues getContentValues(UploadLocationInfo location) {
    ContentValues values = new ContentValues();
    ...
    values.put(LocationEntry.COLUMN_NAME_MESSAGE, json);
    return values;
  }
}
